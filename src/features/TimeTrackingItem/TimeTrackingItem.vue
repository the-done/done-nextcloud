...
<template>
  <div ref="itemRoot" class="flex items-center relative">
    <DragVertical
      v-if="draggable"
      :size="20"
      data-handle
    ...
    <div class="time-tracking-item-shadow w-full lg:max-w-[868px] relative">
      <div
        :class="[
          ...
          'w-full p-1 pb-2 lg:pr-[100px] bg-(--color-background-hover)',
          'border rounded-(--border-radius-element) transition-all duration-200',
          highlight
...
import { VChip } from "@/shared/components";

export default {
  name: "TimeTrackingItem",
  emits: ["onCopy", "onDelete", "onClearHighlight"],
  components: {
    NcButton,
    NcActions,
    NcActionButton,
  ...
  methods: {
    setupIntersectionObserver() {
      this.observer = new IntersectionObserver(
        (entries) => {
          entries.forEach((entry) => {
            if (entry.isIntersecting && this.highlight === true) {
      ...
      if (this.$refs.itemRoot) {
        this.observer.observe(this.$refs.itemRoot);
      }
    },
    handleCopy() {
