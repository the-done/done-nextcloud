...
import {
  ...
  saveDynamicField,
  deleteDynamicField,
} from "@/entities/dynamicFields/api";
import { getFieldCommentsBySource } from "@/entities/fieldComments/api";
  ...
  methods: {
    ...
    async handleDeleteField({ id }) {
      try {
        ...
        this.$notify({
          text: this.contextTranslate("Field deleted successfully"),
          type: "success",
          duration: 2 * 1000,
        });
      } catch (e) {
    ...
    async handleUpdateField({ id, formValues }) {
      try {
        ...
        this.$notify({
          text: this.contextTranslate("Field updated successfully"),
          type: "success",
          duration: 2 * 1000,
        });
      } catch (e) {
