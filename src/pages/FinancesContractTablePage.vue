      ...
      <template #controls="{ row: { slug, slug_type } }">
        <NcActions :inline="1">
          ...
          <NcActionButton
            v-if="getCommonPermission('canReadFinances') === true"
            :force-name="true"
            :aria-label="contextTranslate('Delete contract', context)"
...
import {
  fetchContractsTableData,
  deleteContractBySlug,
...
export default {
  name: "FinancesContractsTablePage",
  ...
  computed: {
    ...
    source() {
      return MAP_DYNAMIC_TABLE_SOURCES["contract"];
    },
  },
  methods: {
    handleClickEdit({ slug }) {
      this.$router.push({ name: "finances-contract-edit", params: { slug } });
    },
    async handleFetchData() {
      try {
        this.setTableLoading(true);

        const { data } = await fetchContractsTableData();
    ...
    async handleDelete({ slug }) {
      ...
      try {
        await deleteContractBySlug({ slug });
