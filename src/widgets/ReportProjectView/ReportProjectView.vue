...
import ReportActionsUserItem from "@/features/ReportActionsUserItem/ReportActionsUserItem.vue";
import ReportActionsProjectItem from "@/features/ReportActionsProjectItem/ReportActionsProjectItem.vue";
import { TimeTrackingItem } from "@/features/TimeTrackingItem";

import { VTimeChip, VExpandIconButton } from "@/shared/components";

import { contextualTranslationsMixin } from "@/shared/lib/mixins/contextualTranslationsMixin";

import { getJoinString } from "@/shared/lib/helpers/string";
import { isEmptyValue } from "@/shared/lib/helpers/validation";
...
export default {
  ...
  methods: {
    ...
    isWeekend(value) {
      ...
      return (
        [
          this.contextTranslate("Saturday"),
          this.contextTranslate("Sunday"),
        ].includes(lowerCaseValue) === true
      );
    },
    getQuarterTitle(value) {
      switch (value) {
        case "1":
          return this.contextTranslate("Q1");
        case "2":
          return this.contextTranslate("Q2");
        case "3":
          return this.contextTranslate("Q3");
        case "4":
          return this.contextTranslate("Q4");
      }
    },
    getMonthTitle(monthId) {
      const index = Number(monthId) - 1;

      return this.contextTranslate(MONTHS[index]);
    },
    getWeekTitle(weekId) {
      const [_year, weekNumber] = weekId.split("_");

      return this.contextTranslate("{n} week").replace("{n}", weekNumber);
    },
