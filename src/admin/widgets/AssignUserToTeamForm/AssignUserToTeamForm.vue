<template>
  <section v-if="loading === false">
    ...
    >
      ...
      <div class="w-[260px]">
        <VDropdown
          ...
          :required="true"
          :error="errors.role"
          :ariaLabel="contextTranslate('Role in team', context)"
          :placeholder="contextTranslate('Role in team', context)"
    ...
    <NcButton
      ...
      @click="toggleActive"
    >
      {{ contextTranslate("Add employee to team", context) }}
    </NcButton>
    <div class="flex flex-col gap-2" v-else>
      ...
      >
        ...
        {{ contextTranslate("Team roles dictionary is empty.", context) }}
        <RouterLink
          :to="{ name: 'dictionary-team-roles-new' }"
          class="v-link v-link--underline"
        >
          {{ t("done", "Add role") }}
        </RouterLink>
...
const REQUIRED_FIELD = ["user", "role"];
const DICTIONARY_NAME = "rolesInTeamDictionary";

export default {
  name: "AssignUserToTeamForm",
  ...
  computed: {
    ...
    errors() {
      const requiredFields = REQUIRED_FIELD.reduce((accum, key) => {
